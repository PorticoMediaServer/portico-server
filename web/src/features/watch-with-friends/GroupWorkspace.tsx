...
function ConnectionState({ state }: { state: WatchConnectionState | 'idle' }) {
  const Icon = state === 'live' ? Wifi : state === 'failed' ? WifiOff : RefreshCw;
...
function GroupTransport({ group, canHost, busy, onCommand }: {
  ...
  return <section className="watch-sync-rail" aria-label="Group playback controls">
    <div className="watch-now-playing"><span><Play /></span><div><strong>{group.mediaTitle}</strong><span>{group.state === 'playing' ? 'Playing for the group' : 'Paused for the group'}</span></div></div>
    {canHost
      ? <div className="watch-transport-controls">
        <IconButton label="Previous queue item" disabled={busy || !hasPrevious} onClick={() => void onCommand({ action: 'previous' })}><ChevronFirst /></IconButton>
        <IconButton label="Back 10 seconds" disabled={busy} onClick={() => void onCommand({ action: 'seek', positionSeconds: Math.max(0, group.positionSeconds - 10) })}><SkipBack /></IconButton>
        <IconButton label={toggleAction === 'play' ? 'Play for group' : 'Pause for group'} className="watch-primary-transport" disabled={busy} onClick={() => void onCommand({ action: toggleAction, positionSeconds: group.positionSeconds })}>{toggleAction === 'play' ? <Play /> : <Pause />}</IconButton>
        <IconButton label="Forward 30 seconds" disabled={busy} onClick={() => void onCommand({ action: 'seek', positionSeconds: group.positionSeconds + 30 })}><SkipForward /></IconButton>
        <IconButton label="Next queue item" disabled={busy || !hasNext} onClick={() => void onCommand({ action: 'next' })}><ChevronLast /></IconButton>
      </div>
      : <div className="watch-transport-owner"><UsersRound /><span>Playback is controlled by {group.ownerName}</span></div>}
...
function MemberRoster({ group, viewer, busy, onUpdate }: {
  ...
  return <section className="watch-members watch-panel">
    <header><div><h3>People</h3><span>{ready} of {group.members.length} ready</span></div>{ownMember && <div className="watch-readiness-controls"><SecondaryButton selected={ownMember.state === 'ready'} disabled={busy} onClick={() => void onUpdate({ state: 'ready', positionSeconds: group.positionSeconds })}><CircleCheck /> Ready</SecondaryButton><SecondaryButton selected={ownMember.state === 'buffering'} disabled={busy} onClick={() => void onUpdate({ state: 'buffering', positionSeconds: group.positionSeconds })}><RotateCcw /> Buffering</SecondaryButton></div>}</header>
  ...
  return <section className="watch-queue watch-panel">
    ...
    {canHost && <form className="watch-queue-add" onSubmit={submit}>
      <MediaSearchPicker
        ...
        disabled={busy}
      />
      <button type="submit" disabled={busy || !mediaId.trim()}><Plus /> Add</button>
    </form>}
    {group.queue.length === 0
      ? <div className="watch-queue-empty"><ListVideo /><strong>The queue is empty</strong></div>
      : <ol>{group.queue.map((item, index) => {
        ...
        return <li className={current ? 'current' : ''} key={item.mediaId}>
          ...
          {canHost && <div className="watch-queue-actions">
            {!current && <button type="button" disabled={busy} onClick={() => void onPlayNow(item.mediaId)} aria-label={`Play ${item.mediaTitle} now`}><Play /></button>}
            <button type="button" disabled={busy || index === 0} onClick={() => move(index, -1)} aria-label={`Move ${item.mediaTitle} up`}><ArrowUp /></button>
            <button type="button" disabled={busy || index === group.queue.length - 1} onClick={() => move(index, 1)} aria-label={`Move ${item.mediaTitle} down`}><ArrowDown /></button>
            <button type="button" disabled={busy || current} onClick={() => void onRemove(item.mediaId)} aria-label={`Remove ${item.mediaTitle} from queue`}><Trash2 /></button>
          </div>}
        </li>;
      })}</ol>}
  </section>;
...
function GroupSettings({ group, canHost, busy, onUpdate }: {
  ...
  return <section className="watch-group-settings watch-panel">
    ...
    <div className="watch-setting-row"><div><Shuffle /><span><strong>Shuffle</strong><span>{group.shuffleEnabled ? 'Enabled' : 'Disabled'}</span></span></div>{canHost && <button type="button" role="switch" aria-label={group.shuffleEnabled ? 'Disable shuffle' : 'Enable shuffle'} aria-checked={group.shuffleEnabled} disabled={busy} onClick={() => void onUpdate({ shuffleEnabled: !group.shuffleEnabled })}><span /></button>}</div>
    <div className="watch-setting-row repeat"><div><RefreshCw /><span><strong>Repeat</strong><span>{group.repeatMode === 'none' ? 'Off' : group.repeatMode === 'one' ? 'Current item' : 'Entire queue'}</span></span></div>{canHost && <div className="watch-repeat-options">{(['none', 'one', 'all'] as const).map((mode) => <button type="button" className={group.repeatMode === mode ? 'selected' : ''} disabled={busy} key={mode} onClick={() => void onUpdate({ repeatMode: mode })}>{mode === 'none' ? 'Off' : mode === 'one' ? 'One' : 'All'}</button>)}</div>}</div>
  </section>;
}
...
export function GroupWorkspace({
  ...
  if (!joined) return <section className="watch-group-preview">
    ...
    <PrimaryButton disabled={busy === `join:${group.id}`} onClick={() => void actions.join()}>{busy === `join:${group.id}` ? <LoaderCircle className="watch-spin" /> : <UserRoundCheck />} Join group</PrimaryButton>
  </section>;
  ...
  return <div className="watch-active-workspace">
    <header className="watch-group-header">
      ...
      <div className="watch-group-header-actions"><ConnectionState state={connection} /><IconButton label="Refresh group" disabled={isBusy} onClick={() => void actions.refresh()}><RefreshCw /></IconButton>{canHost ? <SecondaryButton disabled={isBusy} onClick={() => setConfirmEnd(true)}>End group</SecondaryButton> : <SecondaryButton disabled={isBusy} onClick={() => void actions.leave()}>Leave group</SecondaryButton>}</div>
    </header>
    {connection === 'failed' && <div className="watch-alert connection" role="alert"><WifiOff /><span><strong>Live updates stopped</strong><span>{connectionError || 'Portico could not reconnect to this group.'}</span></span><SecondaryButton onClick={actions.retryEvents}>Reconnect</SecondaryButton></div>}
