...
import { NavigationMoveUpIcon, NavigationDisclosureIcon, StatusWarningIcon, LibraryCollectionIcon, StatusLockedIcon, ActionAddIcon, DeviceStorageIcon, StatusLoadingIcon, ActionRefreshIcon, DeviceOfflineIcon, ActionCloseIcon } from '#portico-icons';
...
function FailureIcon({ kind }: { kind: FilesystemFailure['kind'] }) {
  if (kind === 'permission') return <StatusLockedIcon />;
  if (kind === 'offline') return <DeviceOfflineIcon />;
...
export function FilesystemPickerDialog({
  ...
  return <ModalOverlay labelledBy="filesystem-picker-title" className="filesystem-picker-dialog" onDismiss={onCancel}>
    <header className="filesystem-picker-header">
      ...
      <IconButton label="Close folder picker" onClick={onCancel}><ActionCloseIcon /></IconButton>
    </header>
    ...
    <div className="filesystem-picker-body">
      <aside className="filesystem-roots" aria-label="Server filesystem roots">
        <header><DeviceStorageIcon /><span>Server mounts</span></header>
        {roots.length > 0
      ...
      <main className="filesystem-directory">
        <div className="filesystem-location-bar">
          <nav aria-label="Folder breadcrumbs">
            {breadcrumbs.map((crumb, index) => <span key={`${crumb.path}:${index}`}>{index > 0 && <NavigationDisclosureIcon />}<button type="button" disabled={loading || sameFilesystemPath(currentPath, crumb.path)} aria-current={sameFilesystemPath(currentPath, crumb.path) ? 'location' : undefined} onClick={() => void loadPath(crumb.path)}>{crumb.label}</button></span>)}
          </nav>
          <div>
            <IconButton label="Open parent folder" disabled={loading || !browse?.parent} onClick={() => browse?.parent && void loadPath(browse.parent)}><NavigationMoveUpIcon /></IconButton>
            <IconButton label="Refresh current folder" disabled={loading || !currentPath} onClick={() => void loadPath(currentPath)}><ActionRefreshIcon className={loading ? 'filesystem-spin' : ''} /></IconButton>
            {canCreateDirectory && <SecondaryButton disabled={loading || creating || !selectedPath} selected={newFolderOpen} onClick={() => { setNewFolderOpen((open) => !open); setCreateError(''); }}><ActionAddIcon /> New folder</SecondaryButton>}
          </div>
        </div>

        <div className="filesystem-current-path" aria-live="polite"><LibraryCollectionIcon /><span><strong>{currentPath ? filesystemPathLabel(currentPath) : 'No folder open'}</strong><span>{currentPath || 'The server did not return a current path.'}</span></span>{currentPath && <small>{directories.length} folder{directories.length === 1 ? '' : 's'}</small>}</div>
        ...
        {newFolderOpen && canCreateDirectory && <form className="filesystem-new-folder" onSubmit={createFolder}>
          ...
          <div><input id="filesystem-new-folder-name" value={newFolderName} onChange={(event) => { setNewFolderName(event.target.value); setCreateError(''); }} disabled={creating} placeholder="Folder name" autoFocus /><SecondaryButton disabled={creating} onClick={() => { setNewFolderOpen(false); setNewFolderName(''); setCreateError(''); }}>Cancel</SecondaryButton><PrimaryButton type="submit" disabled={creating || !newFolderName.trim()}>{creating ? <StatusLoadingIcon className="filesystem-spin" /> : <ActionAddIcon />} {creating ? 'Creating' : 'Create'}</PrimaryButton></div>
          {createError && <p className="filesystem-inline-error" role="alert">{createError}</p>}
        </form>}

        {failure && browse && <div className={`filesystem-notice ${failure.kind}`} role="alert"><FailureIcon kind={failure.kind} /><span><strong>{failureTitle(failure.kind)}</strong><span>{failure.message}</span></span><SecondaryButton onClick={() => void loadPath(lastRequestedPath || undefined)}><ActionRefreshIcon /> Try again</SecondaryButton></div>}
        ...
        <div ref={folderListRef} className="filesystem-folder-list" role="list" aria-label={currentPath ? `Folders in ${currentPath}` : 'Server folders'} aria-busy={loading} onKeyDown={onFolderListKeyDown}>
          {loading && !browse && <div className="filesystem-state" aria-live="polite"><StatusLoadingIcon className="filesystem-spin" /><strong>Loading server folders</strong><p>Reading paths from the Portico server host.</p></div>}
          {initialFailure && <div className={`filesystem-state ${initialFailure.kind}`} role="alert"><FailureIcon kind={initialFailure.kind} /><strong>{failureTitle(initialFailure.kind)}</strong><p>{initialFailure.message}</p><SecondaryButton onClick={() => void loadPath(lastRequestedPath || undefined)}><ActionRefreshIcon /> Try again</SecondaryButton></div>}
          {noFilesystem && <div className="filesystem-state"><DeviceStorageIcon /><strong>No server roots available</strong><p>Enter an absolute path above, or check that the server can read its host filesystem.</p></div>}
          {!loading && browse && currentPath && directories.length === 0 && <div className="filesystem-state empty"><LibraryCollectionIcon /><strong>No folders here</strong><p>You can select this folder, open its parent, or create a child folder if your account allows it.</p></div>}
          {browse && directories.map((entry) => <div className="filesystem-folder-row" role="listitem" key={entry.path}><button type="button" disabled={loading || !entry.readable} aria-label={entry.readable ? `Open folder ${entry.name}` : `Folder ${entry.name} is not readable`} onClick={() => entry.readable && void loadPath(entry.path)}><span className="filesystem-folder-icon">{entry.readable ? <LibraryCollectionIcon /> : <StatusLockedIcon />}</span><span><strong>{entry.name}</strong><span>{entry.path}</span></span><small>{entry.readable ? 'Open' : 'Not readable'}</small><NavigationDisclosureIcon /></button></div>)}
          {loading && browse && <div className="filesystem-loading-overlay" aria-live="polite"><StatusLoadingIcon className="filesystem-spin" /> Opening folder</div>}
        </div>
      </main>
    </div>
