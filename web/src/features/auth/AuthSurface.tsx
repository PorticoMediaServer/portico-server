...
} from "../../runtime/hostedAvailability";
...
function AuthBrand() {
  return (
    <img
      ...
      src="/brand/portico-wordmark-white.svg"
      alt="Portico"
    />
  );
}
...
function AccountLegalNotice() {
  return (
    <p className="auth-account-legal">
      By continuing, you agree to Portico’s{" "}
      <a href="https://getportico.tv/terms/" target="_blank" rel="noreferrer">
        Terms of Use
      </a>{" "}
      and acknowledge the{" "}
      <a href="https://getportico.tv/privacy/" target="_blank" rel="noreferrer">
        Privacy Policy
      </a>
      .
    </p>
...
function productErrorText(reason: unknown, fallback: ProductMessageId): string {
  ...
  const presentation =
    resolved.id === "problem.request-failed" &&
    fallback !== "problem.request-failed"
      ? productMessage(fallback)
      : resolved;
  return (
    presentation.body ??
    presentation.text ??
    presentation.title ??
    productMessage("problem.request-failed").body ??
    "Portico could not complete this request."
  );
}
...
function setupClaimErrorText(reason: unknown): string {
  const candidate =
    reason && typeof reason === "object"
      ? (reason as { code?: unknown; retryable?: unknown })
      : undefined;
  ...
  if (
    ...
    return "This setup can no longer be resumed. Return to Account options and start a new Portico Account setup.";
  }
  if (code === "claim_name_locked")
    return "Finish or cancel the current Portico Account setup before changing this server name.";
  if (
    candidate?.retryable === true ||
    [
      "hosted_claim_unreachable",
      "hosted_claim_rejected",
      "hosted_claim_failed",
    ].includes(code)
  )
    return "Portico Account services are temporarily unavailable. This server will keep trying automatically.";
...
const demoPassword = String(
  import.meta.env.VITE_PORTICO_DEMO_PUBLIC_PASSWORD ?? "",
);

export function AuthLoadingSurface({
  title = "Opening your server",
  detail = "Checking your session and server access…",
}: {
  title?: string;
  detail?: string;
}) {
  ...
  return (
    <main className="auth-surface">
      <section
        className="auth-panel auth-loading"
        aria-busy={!delayed}
        aria-live="polite"
      >
        <AuthBrand />
        <div className="auth-loading-state">
          ...
          <span>
            ...
            <p>
              {delayed
                ? "The server is taking longer than expected to answer."
                : detail}
            </p>
          </span>
        </div>
    ...
    <main className="auth-surface">
      <section className="auth-panel account-chooser-panel">
        ...
        <header className="auth-heading">
          ...
          <p>Choose an account remembered for this server.</p>
        </header>
        {(switchError || error) && (
        ...
        <div className="account-chooser-list">
          {data.accounts.map((account) => {
            ...
            return (
              ...
              >
                <span className="account-chooser-avatar">
...
export function LocalProfileSelectionSurface({
  serverName,
}: {
  serverName: string;
}) {
  ...
  const profiles = [...challenge.directory.profiles].sort(
  ...
  const open = async (profileId: string, profilePin?: string) => {
    setError("");
    try {
      await auth.selectLocalProfile(profileId, profilePin);
    } catch (reason) {
      ...
      <section className="auth-panel account-chooser-panel">
        ...
        <div className="account-chooser-list">
          {profiles.map((profile) => (
            ...
            >
              <span className="account-chooser-avatar">
                {profile.name.trim().slice(0, 1).toLocaleUpperCase() || "P"}
              </span>
              <span>
        ...
        {selected?.hasPIN && (
          <form
            className="runtime-profile-pin"
            onSubmit={(event) => {
              event.preventDefault();
              void open(selected.id, pin);
            }}
          >
            <label>
              <span>
              ...
              <input
                autoFocus
                aria-label={`${selected.name} profile PIN`}
                inputMode="numeric"
            ...
            <PrimaryButton
              type="submit"
              disabled={auth.busy || pin.length !== 4}
            >
              {auth.busy ? "Opening…" : "Open profile"}
            </PrimaryButton>
          </form>
        )}
        <div className="account-chooser-actions">
...
export function SignInSurface({
  ...
  const [error, setError] = useState("");
  const callbackWarning = runtime?.connectionWarning
    ? productMessage(runtime.connectionWarning)
    : undefined;
  ...
  return (
    <main className="auth-surface">
      <section className="auth-panel auth-sign-in-panel">
        ...
        <header className="auth-heading">
          <h1>
            {addingAccount ? "Add an account" : `Sign in to ${serverName}`}
          </h1>
          <p>
            {addingAccount
              ? "Sign in once to make another account available from this browser."
              : porticoEnabled && !localEnabled
                ? "Use the Portico Account that has access to this server."
                : "Use an account managed by this server."}
          </p>
        </header>
        {callbackWarning && (
          <p
        ...
        {demoMode &&
          ...
          demoPassword && (
            <aside
              className="auth-demo-credentials"
              aria-label="Public demo credentials"
            >
              <header>
                <strong>Public demo access</strong>
                <small>Direct Play only · transcoding is disabled</small>
              </header>
              <dl>
                <div>
                  <dt>Username</dt>
        ...
        <label className="auth-remember-account">
          <input
            type="checkbox"
            checked={rememberOnBrowser}
            onChange={(event) => setRememberOnBrowser(event.target.checked)}
          />
          <span>Remember this account on this browser</span>
        </label>
        {porticoEnabled && (
          <a
        ...
        {!localEnabled && !porticoEnabled && (
          <div className="auth-method-error" role="alert">
            ...
            <span>
              ...
              <p>
                Ask the server owner to finish setup or restore a supported
                sign-in method.
              </p>
            </span>
          </div>
        ...
        {addingAccount && (
          <button
            className="auth-text-button auth-cancel-add"
            type="button"
            disabled={auth.busy}
...
export function SetupSurface({ serverName }: { serverName: string }) {
  ...
  const savedDraft =
    typeof window !== "undefined"
      ? window.sessionStorage.getItem("portico.server-setup-draft.v1")
      : null;
  const parsedDraft = (() => {
    try {
      return savedDraft
  ...
  const [mode, setMode] = useState<"choose" | "local" | "portico">(
    returningFromPortico ? "portico" : (parsedDraft?.mode ?? "choose"),
  );
  const [setupServerName, setSetupServerName] = useState(
    parsedDraft?.serverName?.trim() || serverName,
  );
  ...
  useEffect(() => {
    ...
    try {
      window.sessionStorage.setItem(
        "portico.server-setup-draft.v1",
        JSON.stringify({ serverName: setupServerName, mode }),
      );
    } catch {
      // Setup remains usable when browser storage is disabled.
    }
  }, [mode, setupServerName]);
    ...
    const check = async () => {
      try {
        ...
        if (!status.setupRequired && status.porticoConnected) {
          window.sessionStorage.removeItem("portico.server-setup-draft.v1");
        ...
        if (
          ...
          setError(
            "This setup request expired before the server could finish activating. Return to account options and try again.",
          );
          return;
        }
  ...
  const submit = async (event: FormEvent) => {
    ...
    setError("");
    try {
      if (!validPorticoPassword(details.password))
        throw new TypeError("Choose a password that meets every requirement.");
      if (details.password !== confirmPassword)
        throw new TypeError("Passwords do not match.");
      const username = details.username.trim();
      await auth.setup({
        ...details,
        serverName: setupServerName.trim(),
        username,
  ...
  const startPorticoSetup = async () => {
    ...
    try {
      if (!setupServerName.trim())
        throw new TypeError("Enter a name for your Portico server.");
      const claim = await auth.startPorticoSetup(setupServerName.trim());
      if (!claim.claimUrl)
        throw new Error(
          "Portico did not return a secure claim address. The server will keep trying automatically.",
        );
    ...
    <main className="auth-surface">
      <section className="auth-panel setup-panel">
        ...
        <header className="auth-heading">
          ...
          <p>Set your server's name and choose a sign-in method.</p>
        </header>
        <div className="setup-server-name">
          <label htmlFor="setup-server-name">
            ...
            <input
              ...
              onChange={(event) => {
                setSetupServerName(event.target.value);
                setError("");
              }}
              required
            />
            <small>
              Shown in Portico applications, invitations, and connection
              screens.
            </small>
          </label>
        </div>
            ...
            {!error && (
              <>
                ...
                <p>
                  ...
                  <small>
                    Portico is activating this server and creating your local
                    owner profile. Remote Access will be checked separately.
                  </small>
                </p>
              </>
            )}
                  ...
                  <span>
                    <strong>
                      {setupAvailability.automatic
                        ? setupAvailability.copy.title
                        : "Portico couldn’t finish setup"}
                    </strong>
                    <small>
                      {setupAvailability.automatic
                        ? setupAvailability.copy.body
                        : error}
                    </small>
                  </span>
                </div>
        ...
        {mode === "choose" && (
          <div className="setup-mode-grid">
            ...
            >
              ...
              <span>
                <span className="setup-mode-title">
                  <strong>Use A Portico Account</strong>
                  <em>Recommended</em>
                </span>
                <small>
                  Using a Portico Account allows you to easily set up and access
                  your server remotely for free, invite other users, and use
                  multiple Portico servers - all in one place. This option
                  provides automatic network setup, including secure remote
                  access to your server.
                </small>
                <small className="setup-mode-guidance">
                  Use this option if you want a seamless experience across
            ...
            <button
              type="button"
              className="setup-mode-secondary"
              disabled={auth.busy}
            ...
            >
              ...
              <span>
                <span className="setup-mode-title">
                  <strong>Sign in directly to a server</strong>
                  <em>Advanced</em>
                </span>
                <small>
                  Connect using your server address and server credentials. No
                  Portico Account is used. This option requires manual
                  configuration of remote access, networking, and security.
                </small>
                <small className="setup-mode-guidance">
                  Use this option if you understand the risks and want complete
            ...
            <form onSubmit={submit}>
              ...
              <label>
                ...
                <PasswordInput
                  ...
                  required
                />
                {confirmPassword && details.password !== confirmPassword && (
                  <small className="setup-password-mismatch" role="status">
                    Passwords do not match.
                  </small>
                )}
              </label>
              <label className="local-only-acknowledgement">
                <input
                  type="checkbox"
                  checked={localOnlyAcknowledged}
                  onChange={(event) =>
                    setLocalOnlyAcknowledged(event.target.checked)
                  }
                  required
                />
                <span>
                  I understand that This Server credentials are managed here and
                  do not create a Portico Account.
                </span>
              </label>
              {error && (
                <p className="auth-error" role="alert">
              ...
              <PrimaryButton
                ...
                disabled={
                  auth.busy ||
                  !setupServerName.trim() ||
                  !localOnlyAcknowledged ||
                  !details.username.trim() ||
                  !validPorticoPassword(details.password) ||
                  details.password !== confirmPassword
                }
              >
                {auth.busy ? "Creating owner…" : "Create This Server owner"}
              </PrimaryButton>
            </form>
            <AccountLegalNotice />
          </>
