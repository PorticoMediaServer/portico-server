...
export function SavedTargetDialog({
  kind,
  mediaIds,
  onDismiss,
}: {
  ...
  const add = async (id: string, name: string, updatedAt: string, itemCount: number) => {
    ...
    try {
      ...
      setError(
        reviewedProductErrorText(reason, "catalog.action-failed", {
          actionName: `update this ${label}`,
        }),
      );
    } finally {
      setBusy("");
  ...
  const create = async () => {
    ...
    setError("");
    try {
      const resource = await mutations.create(kind, {
        title: title.trim(),
        visibility: "private",
        mediaIds,
      });
      setComplete(
        `Created ${resource.title} with ${mediaIds.length} ${mediaIds.length === 1 ? "item" : "items"}.`,
      );
      setTitle("");
      setCreating(false);
      setReloadKey((value) => value + 1);
    ...
    >
      ...
      <div className="saved-target-body">
        ...
        {resources.status === "error" && (
          <div className="saved-target-state error" role="alert">
            {reviewedProductErrorText(resources.error, "media.load-failed", {
              featureName: "Saved lists",
            })}
          </div>
        )}
        {resources.status === "success" && resources.data.length > 0 && (
          <div className="saved-target-list">
            {resources.data.map((resource) => (
              ...
              >
                ...
                <span>
                  ...
                  <small>
                    {resource.itemCount}{" "}
                    {resource.itemCount === 1 ? "item" : "items"} ·{" "}
                    {resource.visibility === "server" ? "Shared" : "Private"}
                  </small>
                </span>
                {busy === resource.id ? (
        ...
        {resources.status === "success" &&
          ...
          !creating && (
            <div className="saved-target-state">
              <Icon /> No {label}s yet
            </div>
          )}
        {creating && (
          <form
            className="saved-target-create"
            onSubmit={(event) => {
          ...
          >
            ...
            <div>
              ...
              <PrimaryButton type="submit" disabled={busy === "new"}>
                {busy === "new" ? "Creating…" : `Create ${label}`}
              </PrimaryButton>
            </div>
          </form>
        )}
        {error && (
          <p className="context-action-error" role="alert">
            {error}
          </p>
        )}
      </div>
...
function PeopleEditor({
  ...
  people: MediaPerson[];
  action: ReactNode;
  onChange: (people: MediaPerson[]) => void;
}) {
  const update = (index: number, patch: Partial<MediaPerson>) =>
    onChange(
      people.map((person, candidate) =>
        candidate === index ? { ...person, ...patch } : person,
      ),
    );
  return (
    <div className="metadata-people-editor">
      <div className="metadata-lock-heading">
        <span>
          <strong>Cast &amp; Crew</strong>
          <small>
            People saved here override scanner-provided credits for this item.
          </small>
        </span>
        {action}
      </div>
      <div className="metadata-people-list">
        {people.map((person, index) => (
          <div key={`${index}:${person.id ?? person.name}`}>
            <label>
              <span>Name</span>
              <input
                value={person.name}
            ...
            <IconButton
              label={`Remove ${person.name || "credit"}`}
              onClick={() =>
...
function valuesFromItem(item: MediaItem): EditorValues {
  return {
    ...
    communityRating:
      item.communityRating != null ? String(item.communityRating) : "",
...
export function MediaMetadataEditor({
  ...
  const [refreshableFields, setRefreshableFields] = useState<string[]>([]);
  ...
  const mark = (field: keyof EditorValues, value: string) => {
    ...
    if (!refreshableFields.includes(field)) {
  ...
  const markPeople = (next: MediaPerson[]) => {
    setPeople(next);
    setTouched((current) =>
      current.includes("people") ? current : [...current, "people"],
    );
    if (!refreshableFields.includes("people")) {
      setLockedFields((current) =>
        current.includes("people") ? current : [...current, "people"],
      );
      setLocksTouched(true);
  ...
  const markTypedMetadata = (key: string, value: string) => {
    ...
    setTouched((current) =>
      current.includes("typedMetadata")
        ? current
        : [...current, "typedMetadata"],
    );
    if (!refreshableFields.includes("typedMetadata")) {
      setLockedFields((current) =>
        current.includes("typedMetadata")
          ? current
          : [...current, "typedMetadata"],
  ...
  const toggleLock = (field: string) => {
    const wasLocked = lockedFields.includes(field);
    setLockedFields((current) =>
      current.includes(field)
        ? current.filter((value) => value !== field)
        : [...current, field],
    );
    setRefreshableFields((current) =>
      wasLocked
        ? current.includes(field)
          ? current
          : [...current, field]
  ...
  const field = (key: keyof EditorValues, label: string, lockable = single) => (
    <label
      className={`metadata-field ${touched.includes(key) ? "touched" : ""}`}
    >
      <span>
        {label}
        {lockable && lockControl(key, label)}
      </span>
      <input
        value={values[key]}
        placeholder={single ? "" : "Mixed · leave unchanged"}
        onChange={(event) => mark(key, event.target.value)}
      />
    </label>
  );
  const save = async () => {
    if (!touched.length && !locksTouched) {
      setError("Change at least one field before saving.");
      return;
    }
    const patch: MediaMetadataUpdate = {};
    if (single) {
      if (first?.metadataRevision == null) {
        setError("Reload this item before saving metadata.");
        return;
      }
      patch.expectedRevision = first.metadataRevision;
    }
    for (const key of touched) {
      if (key === "genres") patch.genres = genres;
        ...
        if (!Number.isInteger(value) || value < 0 || value > maximum) {
          setError(
            `Enter a valid ${key.replace(/([A-Z])/g, " $1").toLocaleLowerCase()}.`,
          );
          return;
        }
        ...
        if (!Number.isFinite(value) || value < 0 || value > 10) {
          setError("Enter a community rating between 0 and 10.");
          return;
        }
        patch.communityRating = value;
      } else {
        (patch as Record<string, unknown>)[key] =
          values[key as keyof EditorValues];
      }
    }
    ...
    try {
      ...
      setError(
        reviewedProductErrorText(reason, "media.update-failed", {
  ...
  const searchMatches = async () => {
    ...
    try {
      setMatches(
        await mutations.searchMatches(
          mediaIds[0],
          matchQuery.trim() || first?.title || "",
        ),
      );
    } catch (reason) {
      setError(
        reviewedProductErrorText(reason, "media.search-failed", {
          featureName: "Metadata",
        }),
      );
    } finally {
  ...
  const applyMatch = async (candidate: MediaMatchCandidate) => {
    if (first?.metadataRevision == null) {
      setError("Reload this item before applying a match.");
      return;
    }
    setBusy(true);
    setError("");
    try {
      await mutations.applyMatch(
        mediaIds[0],
        candidate,
  ...
  const uploadArtwork = async (type: string, file: File) => {
    if (first?.metadataRevision == null)
      throw new Error("Reload this item before changing artwork.");
  ...
  const deleteArtwork = async (imageId: string) => {
    if (first?.metadataRevision == null)
      throw new Error("Reload this item before changing artwork.");
    await mutations.deleteArtwork(mediaIds[0], imageId, first.metadataRevision);
    mediaUpdated();
  };
  const preferArtwork = async (imageId: string) => {
    if (first?.metadataRevision == null)
      throw new Error("Reload this item before changing artwork.");
    await mutations.setPreferredArtwork(
      mediaIds[0],
      imageId,
      first.metadataRevision,
    );
  ...
  const reorderArtwork = async (imageIds: string[]) => {
    if (first?.metadataRevision == null)
      throw new Error("Reload this item before changing artwork.");
    await mutations.reorderArtwork(
      mediaIds[0],
      imageIds,
      first.metadataRevision,
    );
  ...
  const normalizedEvidence = [
    ...
    .filter((entry) => catalogEvidenceKinds.has(entry.kind) && entry.value)
    .slice(0, 24);
  const headingId = "media-metadata-editor-title";
  if (single && detail.status === "loading" && !first)
    return (
      <ModalOverlay
        labelledBy={headingId}
        className="metadata-editor metadata-loading"
        onDismiss={onDismiss}
      >
  ...
  if (single && detail.status === "error" && !first)
    return (
      <ModalOverlay
        labelledBy={headingId}
        className="metadata-editor metadata-loading"
      ...
      >
        ...
        <div className="library-state error" role="alert">
          <strong>Metadata is unavailable</strong>
          <p>
            {reviewedProductErrorText(detail.error, "media.load-failed", {
              featureName: "Metadata",
            })}
          </p>
    ...
    >
      <header className="metadata-header">
        <div>
          ...
          <h1 id={headingId}>Edit metadata</h1>
          {single && first && <span>{first.title}</span>}
        </div>
        <IconButton
          label="Close metadata editor"
          disabled={busy}
          onClick={onDismiss}
        >
          <ActionCloseIcon />
      ...
      <div className="metadata-body">
        <nav className="metadata-tabs" aria-label="Metadata sections">
          {tabs.map((name) => (
            <button
              type="button"
              key={name}
        ...
        <div className="metadata-panel">
          {tab === "General" && (
            <>
              <div className="metadata-form-grid">
                {single && field("title", "Title")}
                {single && field("sortTitle", "Sort title")}
                {single && field("originalTitle", "Original title")}
                {single && field("edition", "Edition")}
                {field("year", "Year")}
                {field("contentRating", "Content rating")}
                {single && field("communityRating", "Community rating (0–10)")}
                {single && field("criticRating", "Critic rating (0–100)")}
                {field("studio", "Studio")}
              ...
              {single && (
                <label className="metadata-field wide">
                  <span>Tagline{lockControl("tagline", "Tagline")}</span>
                  <input
                    value={values.tagline}
                    onChange={(event) => mark("tagline", event.target.value)}
                  />
                </label>
              )}
              <label className="metadata-field wide">
                <span>
                  Summary{single && lockControl("summary", "Summary")}
                </span>
                <textarea
                  value={values.summary}
                  placeholder={single ? "" : "Mixed · leave unchanged"}
                  onChange={(event) => mark("summary", event.target.value)}
                />
              </label>
              {single && Object.keys(typedMetadata).length > 0 && (
                <div className="metadata-stack">
                  <div className="metadata-lock-heading">
                    <span>
                      <strong>Additional metadata</strong>
                      <small>
                        Provider-specific fields stored with this item.
                      </small>
                    </span>
                    {lockControl("typedMetadata", "Additional metadata")}
                  </div>
                  <div className="metadata-form-grid">
                    {Object.entries(typedMetadata)
                      .sort(([left], [right]) => left.localeCompare(right))
                      .map(([key, value]) => (
                        <label className="metadata-field" key={key}>
                          <span>
                            {key.replace(/([A-Z])/g, " $1")}
                            {lockControl(`typedMetadata.${key}`, key)}
                          </span>
                          <input
                            value={value}
                            onChange={(event) =>
                              markTypedMetadata(key, event.target.value)
                            }
                          />
                        </label>
                      ))}
            ...
            <div className="metadata-stack">
              <div className="metadata-lock-heading">
                <span>
                  ...
                  <small>
                    Lock artwork to keep scanners from replacing the selected
                    images.
                  </small>
                </span>
                {lockControl("artwork", "Artwork")}
              </div>
              <ArtworkEditor
                images={first.mediaImages ?? []}
                fallbackUrls={{
                  poster: first.poster,
                  backdrop: first.backdrop,
                  thumb: first.backdrop,
                }}
                onUpload={uploadArtwork}
                onDelete={deleteArtwork}
                onPreferred={preferArtwork}
                onReorder={reorderArtwork}
              />
            </div>
          )}
          {tab === "Media" && first && (
            <TechnicalMediaEditor
              item={first}
              attachments={first.attachments}
              loadOptions={loadTechnicalOptions}
              onUploadSubtitle={async (file, language, label) => {
                await mutations.uploadSubtitle(first.id, file, language, label);
                mediaUpdated();
              }}
              onUpdateSubtitle={async (streamId, offsetMs) => {
                await mutations.updateSubtitle(first.id, streamId, offsetMs);
                mediaUpdated();
              }}
              onDeleteSubtitle={async (streamId) => {
                await mutations.deleteSubtitle(first.id, streamId);
                mediaUpdated();
              }}
              onCreateVersion={async (profile) => {
                await operations.createOptimizedVersion(first.id, profile);
                mediaUpdated();
              }}
              onDeleteVersion={async (profile) => {
                await operations.deleteOptimizedVersion(first.id, profile);
                mediaUpdated();
              }}
            />
          )}
          {tab === "Lyrics" && first?.entityKind === "track" && (
            <LyricsEditor
              ...
              defaultQuery={[first.title, first.subtitle.split(" · ")[0]]
                .filter(Boolean)
                .join(" ")}
              onUpload={async (file, language) => {
                await mutations.uploadLyrics(first.id, file, language);
                mediaUpdated();
              }}
              onFetch={async () => {
                await mutations.fetchLyrics(first.id);
                mediaUpdated();
              }}
              onSearch={(query) => mutations.searchLyrics(first.id, query)}
              onApply={async (candidate) => {
                await mutations.applyLyrics(first.id, candidate);
                mediaUpdated();
              }}
              onDelete={async (lyricId) => {
                await mutations.deleteLyrics(first.id, lyricId);
                mediaUpdated();
              }}
            />
          )}
          {tab === "Tags" && (
            <div className="metadata-stack">
              <TagEditor
                action={single && lockControl("genres", "Genres")}
                label="Genres"
                values={genres}
                onChange={(next) => markList("genres", next)}
              />
              <TagEditor
                action={single && lockControl("tags", "Tags")}
                label="Tags"
                values={tags}
                onChange={(next) => markList("tags", next)}
              />
              <TagEditor
                action={single && lockControl("labels", "Labels")}
                label="Labels"
                values={labels}
                onChange={(next) => markList("labels", next)}
              />
            </div>
          )}
          {tab === "Cast & Crew" && single && (
            <PeopleEditor
              people={people}
              action={lockControl("people", "Cast & Crew")}
              onChange={markPeople}
            />
          )}
          {tab === "Matching" && (
            <div className="matching-panel">
              {(acceptedIdentities.length > 0 ||
                ...
                <section
                  className="metadata-source-summary"
                  aria-label="Current metadata sources"
                >
                  <div className="metadata-lock-heading">
                    <span>
                      <strong>Current metadata sources</strong>
                      <small>
                        Accepted identities and revision{" "}
                        {first?.metadataEvidence?.revision ?? "—"} normalized
                        catalog evidence.
                      </small>
                    </span>
                  </div>
                  {acceptedIdentities.length > 0 && (
                    <div
                      className="metadata-evidence-list"
                      aria-label="Accepted provider identities"
                    >
                      {acceptedIdentities.map((identity) => {
                        const provider = identity.provider.toLocaleLowerCase();
                        const attribution =
                          provider === "tvdb"
                            ? { label: "TheTVDB", href: "https://thetvdb.com/" }
                            : provider === "tmdb"
                              ? {
                                  label: "TMDB",
                        ...
                        return (
                          <span
                            key={`${identity.provider}:${identity.externalType}:${identity.externalId}`}
                          >
                            <strong>
                              {attribution ? (
                                <a
                                ...
                                identity.provider.toLocaleUpperCase()
                              )}
                            </strong>
                            <small>
                              {identity.externalType} · {identity.externalId}
                            </small>
                          </span>
                        );
                      })}
                  ...
                  {normalizedEvidence.length > 0 && (
                    <>
                      <div className="metadata-lock-heading">
                        <span>
                          <strong>Search and filter metadata</strong>
                          <small>
                            Normalized fields available to catalog discovery
                            where that field applies.
                          </small>
                        </span>
                      </div>
                ...
                <div className="match-results">
                  {matches.map((candidate) => (
                    ...
                    >
                      ...
                      <span>
                        <strong>
                          {candidate.title || candidate.externalId}
                        </strong>
                        <small>
                          {[
                            candidate.year,
                            `${candidate.provider.toLocaleUpperCase()} ${candidate.externalType} ${candidate.externalId}`,
                            candidate.accepted ? "accepted" : "candidate",
                            `${Math.round(Math.min(1, Math.max(0, candidate.score)) * 100)}% match`,
                          ]
                            .filter(Boolean)
                            .join(" · ")}
                        </small>
                        {candidate.reasons.length > 0 && (
                          <small>
                            Evidence: {candidate.reasons.map((reason) => reason.detail || reason.code).join(", ")}
                          </small>
                        )}
                        <small>Observed {new Date(candidate.createdAt).toLocaleString()}</small>
      ...
      <footer className="metadata-footer">
        <span>
          {immediateTab
            ? pendingChanges
              ? `${pendingChanges} metadata ${pendingChanges === 1 ? "change" : "changes"} pending · ${tab} saves immediately`
              : `${tab} changes save immediately`
            : pendingChanges
              ? `${pendingChanges} pending ${pendingChanges === 1 ? "change" : "changes"}`
              : "No changes yet"}
        </span>
        <SecondaryButton disabled={busy} onClick={onDismiss}>
          {immediateTab && !pendingChanges ? "Close" : "Cancel"}
        </SecondaryButton>
        {(!immediateTab || pendingChanges > 0) && (
          <PrimaryButton disabled={busy} onClick={() => void save()}>
            {busy
              ? "Saving…"
              : single
                ? "Save changes"
                : `Apply to ${mediaIds.length} items`}
          </PrimaryButton>
        )}
      </footer>
    </ModalOverlay>
