...
describe('owner library scan operations', () => {
  ...
  it('lets the owner add a WebDAV source without retaining credentials in the rendered settings state', async () => {
    ...
    fireEvent.change(within(dialog).getByRole('combobox', { name: 'Remote scan depth' }), { target: { value: 'custom' } });
    expect(within(dialog).getByText(/Custom is advanced.*sustained\/full-file reads.*generated storage/)).toBeInTheDocument();
    fireEvent.change(within(dialog).getByRole('combobox', { name: 'Remote scan depth' }), { target: { value: 'basic' } });
  ...
  it('validates rclone input, queues inventory, and requires confirmation before removal', async () => {
    ...
    fireEvent.change(depth, { target: { value: 'custom' } });
    await waitFor(() => expect(updateMode).toHaveBeenCalledWith('fixture-movies', 'remote-rclone', 'custom', expect.any(AbortSignal)));
    expect(await within(remote).findByText(/Custom is advanced.*sustained\/full-file reads.*generated storage/)).toBeInTheDocument();
