...
import { StatusWarningIcon, StatusSuccessIcon, PlaybackTechnicalStatsIcon, PlaybackQualityIcon, DeviceStorageIcon, ActionRefreshIcon, NavigationSettingsIcon } from '#portico-icons';
...
function RuntimeCapacity({ source }: { source: SettingsDataSource }) {
  ...
  return <SettingsGroup title="Playback runtime" description="Transcode capacity, hardware support, and required runtime dependencies." actions={<SecondaryButton onClick={() => setRevision((current) => current + 1)}><ActionRefreshIcon /> Refresh</SecondaryButton>}>
    ...
    <div className="portico-playback-ledger">
      <div><PlaybackQualityIcon /><span><small>Sessions</small><strong>{capacity.activeSessions} active · {capacity.availableSlots} available</strong><em>{capacity.maxConcurrentSessions} maximum</em></span></div>
      <div><PlaybackTechnicalStatsIcon /><span><small>Hardware</small><strong>{capacity.hardwareSupportLevel.replace('_', ' ')}</strong><em>{capacity.hardwareEncoderAvailable ? capacity.hardwareEncoder || 'Encoder available' : 'Software encoding'}</em></span></div>
      <div><NavigationSettingsIcon /><span><small>HDR tone mapping</small><strong>{capacity.hdrToneMappingStatus}</strong><em>{capacity.hdrToneMappingDetail}</em></span></div>
      <div><DeviceStorageIcon /><span><small>Temporary storage</small><strong>{capacity.temporaryDirectoryReady ? 'Ready' : 'Unavailable'}</strong><em>{capacity.temporaryDirectory}</em></span></div>
    </div>
    <div className="portico-runtime-dependencies">
      {[capacity.ffmpeg, capacity.ffprobe].map((dependency) => <div key={dependency.name}><span className={dependency.available ? 'healthy' : 'danger'}>{dependency.available ? <StatusSuccessIcon /> : <StatusWarningIcon />}</span><span><strong>{dependency.name}</strong><small>{dependency.available ? dependency.versionLine || dependency.resolvedPath || 'Available' : dependency.error || `${dependency.configuredPath} was not found`}</small></span></div>)}
    </div>
  </SettingsGroup>;
}
...
function OptimizedStorage({ source }: { source: SettingsDataSource }) {
  ...
  if (!optimized) return <SettingsGroup title="Optimized version storage" description="Server-wide usage for generated compatible copies."><div className="portico-settings-state"><DeviceStorageIcon /><strong>No optimized storage category</strong><p>This server did not report a managed optimized-version path.</p></div></SettingsGroup>;
  return <SettingsGroup title="Optimized version storage" description="Server-wide usage for generated compatible copies." actions={optimized.cleanupSupported ? <SecondaryButton disabled={mutation.busy} onClick={() => void cleanup()}><ActionRefreshIcon className={mutation.busy ? 'portico-settings-spinner' : ''} /> {mutation.busy ? 'Cleaning…' : 'Run managed cleanup'}</SecondaryButton> : undefined}>
...
export function PlaybackOperations({ source, viewer }: { source: SettingsDataSource; viewer: SettingsViewer }) {
  ...
  if (!canManage) return <SettingsGroup title="Playback operations" description="Runtime capacity and managed optimized storage."><div className="portico-settings-readonly-note"><NavigationSettingsIcon />Your account can view playback preferences but cannot inspect server runtime details.</div></SettingsGroup>;
