...
import { StatusSuccessIcon, ActionOpenExternalIcon, DeviceNetworkIcon, DeviceOfflineIcon, ActionRefreshIcon, StatusSecureIcon } from '#portico-icons';
...
export function RemoteAccessSettingsPanel({ source, viewer }: { source: SettingsDataSource; viewer: SettingsViewer }) {
  ...
  return <div className="portico-settings-form portico-remote-access">
    ...
    {!claimed && <SettingsGroup title="Portico claim" description="Link this server to Portico for free direct remote access and hosted sign-in.">
      {remote.claim && claimLink ? <div className="portico-claim-state"><span><strong>Claim is waiting</strong><small>Expires {new Date(remote.claim.expiresAt).toLocaleString()}</small></span><a className="button primary" href={claimLink} target="_blank" rel="noreferrer">Continue at Portico <ActionOpenExternalIcon /></a><SecondaryButton disabled={mutation.busy} onClick={() => void action((signal) => source.cancelRemoteAccessClaim(signal), 'Claim cancelled.')}>Cancel</SecondaryButton></div> : <SettingRow label="Link a Portico account" description="Creates a short-lived claim. Your server credentials and media stay on this server."><PrimaryButton disabled={!canEdit || mutation.busy} onClick={() => void action((signal) => source.startRemoteAccessClaim(signal), 'Claim started. Continue at Portico to approve it.')}><StatusSecureIcon /> Start claim</PrimaryButton></SettingRow>}
    </SettingsGroup>}
    ...
    <SettingsGroup title="Certificate" description="TLS identity for the public direct route.">
      ...
      {!value('customCertificateEnabled') && claimed && <SettingRow label="Managed certificate" description={`${settings.certificateStatus}${settings.certificateExpiresAt ? ` · Expires ${new Date(settings.certificateExpiresAt).toLocaleDateString()}` : ''}`}><SecondaryButton disabled={!canEdit || mutation.busy} onClick={() => void action((signal) => source.renewRemoteAccessCertificate(signal), 'Certificate renewal requested.')}><ActionRefreshIcon /> Renew now</SecondaryButton></SettingRow>}
    </SettingsGroup>
    <SaveBar dirty={dirty} busy={mutation.busy} feedback={feedback} error={error} onSave={save} onReset={() => { setDraft({}); setFeedback(''); setError(''); }} />
    {claimed && viewer.role === 'owner' && <SettingsGroup title="Remove Portico claim" description="Disconnect hosted account authorization and the assigned Portico route. Local server accounts continue to work."><SettingRow label="Unclaim this server" description="Existing Portico remote sessions and hosted membership mapping will stop working.">{confirmUnclaim ? <div className="portico-confirm-actions"><SecondaryButton disabled={mutation.busy} onClick={() => setConfirmUnclaim(false)}>Cancel</SecondaryButton><button type="button" className="button secondary portico-destructive-button" disabled={mutation.busy} onClick={() => void action((signal) => source.unclaimRemoteAccess(signal), 'Server claim removed.').then(() => setConfirmUnclaim(false))}><DeviceOfflineIcon /> Unclaim server</button></div> : <SecondaryButton onClick={() => setConfirmUnclaim(true)}><DeviceOfflineIcon /> Unclaim</SecondaryButton>}</SettingRow></SettingsGroup>}
  </div>;
}
