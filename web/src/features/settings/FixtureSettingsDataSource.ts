...
export class FixtureSettingsDataSource implements SettingsDataSource {
  ...
  revokePorticoMemberInvite(inviteId: string): Promise<void> {
    const existing = this.operations.porticoInvites?.find((invite) => invite.id === inviteId);
    if (existing) Object.assign(existing, { status: 'revoked', revokedAt: now() });
    return Promise.resolve();
  }
  ...
  restoreBackup(name: string, _stepUp: Parameters<SettingsDataSource['restoreBackup']>[1], _confirmation: string): Promise<RestoreWorkflowResponse> { return Promise.resolve(asFixture<RestoreWorkflowResponse>({ ok: true, name, operationId: `fixture-restore-${Date.now()}`, state: 'staged', phase: 'staged', progress: 25, instruction: 'Backup is staged for supervised restore; it is not restored yet.', statusToken: `fixture-status-${Date.now()}` })); }
  restoreUploadedDatabase(file: File, _stepUp: Parameters<SettingsDataSource['restoreUploadedDatabase']>[1], _confirmation: string): Promise<RestoreWorkflowResponse> { return Promise.resolve(asFixture<RestoreWorkflowResponse>({ ok: true, name: file.name, operationId: `fixture-restore-${Date.now()}`, state: 'staged', phase: 'staged', progress: 25, sourceKind: 'raw-import', manifestVerified: false, instruction: 'Storage import is staged for supervised restore; it is not restored yet.', statusToken: `fixture-status-${Date.now()}` })); }
