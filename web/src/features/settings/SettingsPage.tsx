...
import { StatusWarningIcon, NavigationDisclosureIcon, ActionRefreshIcon, DeviceServerIcon } from '#portico-icons';
...
function ServerSection({ section, workspace, viewer, source, filesystemSource, operations, canManageServerSettings, onWorkspaceChange, onWorkspaceReload, onOperationsReload }: {
  ...
  if (section === 'maintenance') return <div className="portico-settings-content">{form}<OperationState state={operations} retry={onOperationsReload} label="maintenance data">{(data) => <MaintenanceOperations tasks={data.tasks} backups={data.backups} storage={data.storage} release={data.failures?.release ? undefined : data.release} failures={data.failures} viewer={viewer} source={source} onChanged={onOperationsReload} />}</OperationState></div>;
...
function PersonalSection({ section, viewer, source, productSource, operations, onOperationsReload }: { section: string; viewer: SettingsViewer; source: SettingsDataSource; productSource?: PorticoDataSource; operations: ReturnType<typeof useSettingsQuery<SettingsOperationalSnapshot | null>>; onOperationsReload: () => void }) {
  ...
  return <div className="portico-settings-state error"><StatusWarningIcon /><strong>This settings link is invalid</strong><p>Choose a section from the settings navigation.</p><Link className="button secondary" to="/settings/account">Open account settings</Link></div>;
}
...
export function SettingsPage({ source, productSource, viewer, filesystemSource }: { source: SettingsDataSource; productSource?: PorticoDataSource; viewer: SettingsViewer; filesystemSource?: FilesystemPickerSource }) {
  ...
  if (!routeIsKnown) return <div className="portico-settings-page"><div className="portico-settings-state error" role="status"><StatusWarningIcon /><strong>Settings section not found</strong><p>The address does not match a settings section available in Portico.</p><Link className="button secondary" to="/settings/account">Open account settings</Link></div></div>;
  if (serverRouteDenied) return <div className="portico-settings-page"><div className="portico-settings-state error" role="status"><StatusWarningIcon /><strong>Server settings aren’t available</strong><p>Your account can use personal settings, but it cannot administer this server.</p><Link className="button secondary" to="/settings/account">Open personal settings</Link></div></div>;
  ...
  return <div className="portico-settings-page">
    <header className="portico-settings-header"><nav aria-label="Breadcrumb"><span>Settings</span><NavigationDisclosureIcon /><span>{isServer ? 'This Server' : 'Personal'}</span></nav><div><h1>{title}</h1><p>{description}</p></div>{isServer && section !== 'status' && currentWorkspace && <span className="portico-settings-document-meta"><DeviceServerIcon /> Updated {new Date(currentWorkspace.document.updatedAt).toLocaleString()}</span>}</header>
    <SettingsSaveCoordinator><div className="portico-settings-layout">
      ...
      <section className="portico-settings-workspace" aria-label={`${title} settings`}>
        {isServer && currentWorkspace && workspace.status === 'error' && <div className="portico-settings-inline-error"><StatusWarningIcon />Unable to refresh the settings document. Showing the last loaded revision.<button type="button" onClick={() => setWorkspaceRevision((current) => current + 1)}><ActionRefreshIcon /> Retry</button></div>}
