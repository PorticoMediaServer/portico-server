...
export function resolveRuntimeConfig(environment: Record<string, string | boolean | undefined>): RuntimeConfig {
  ...
  const defaultHostedApiBaseUrl = mode === 'hosted' ? 'https://web.getportico.tv' : 'https://api.getportico.tv';
  const hostedApiBaseUrl = String(environment.VITE_PORTICO_HOSTED_API_URL || defaultHostedApiBaseUrl).replace(/\/+$/, '');
