...
func TestProviderIdentityConflictPreservesHistoryAndManualAcceptanceSupersedes(t *testing.T) {
	...
	defer tx.Rollback()
	if err := upsertMediaProviderIdentityTx(tx, "media_identity_history", "tmdb", "accepted-first", "movie", .95, "provider-search", false, "", now); err != nil {
		t.Fatalf("accept first automated identity: %v", err)
	}
	if err := upsertMediaProviderIdentityTx(tx, "media_identity_history", "tmdb", "conflicting-second", "movie", .99, "provider-search", false, "", now); err != nil {
		t.Fatalf("record conflicting automated identity: %v", err)
	}

	var firstStatus, secondStatus string
	if err := tx.QueryRow(`SELECT status FROM media_provider_ids WHERE media_id=? AND provider='tmdb' AND external_id='accepted-first'`, "media_identity_history").Scan(&firstStatus); err != nil {
		t.Fatalf("load accepted identity: %v", err)
	}
	if err := tx.QueryRow(`SELECT status FROM media_provider_ids WHERE media_id=? AND provider='tmdb' AND external_id='conflicting-second'`, "media_identity_history").Scan(&secondStatus); err != nil {
	...
	if err := upsertMediaProviderIdentityTx(tx, "media_identity_history", "tmdb", "conflicting-second", "movie", 1, "manual-review", true, "user_reviewer", manualAt); err != nil {
	...
	if err := tx.QueryRow(`SELECT status, accepted_by_user_id FROM media_provider_ids WHERE media_id=? AND provider='tmdb' AND external_id='conflicting-second'`, "media_identity_history").Scan(&secondStatus, &acceptedBy); err != nil {
